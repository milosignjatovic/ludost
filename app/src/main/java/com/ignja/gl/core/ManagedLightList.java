...
public class ManagedLightList 
{
	...
	public void reset() {
		...
		for (int i = 0; i < MyGLRenderer.NUM_GLLIGHTS; i++) {
			_availGlIndices.add(i);
		}

		_lightToGlIndex = new HashMap<Light, Integer>();

		_glIndexEnabled = new boolean[MyGLRenderer.NUM_GLLIGHTS];
		_glIndexEnabledDirty = new boolean[MyGLRenderer.NUM_GLLIGHTS];
		for (int i = 0; i < MyGLRenderer.NUM_GLLIGHTS; i++) {
	...
	public boolean add(Light $light) 
	{
		...
		if (_lights.size() > MyGLRenderer.NUM_GLLIGHTS)
